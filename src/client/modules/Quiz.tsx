...
export const Quiz: React.FC<PropsInterface> = (props) => {
  ...
  useEffect(() => {
    ...
    if (currentQuestionType === "boolean") {
      const questionObject = {
        question: props.bool[booleanCount]
          ? props.bool[booleanCount].question
          : null,
        correctAnswer: props.bool[booleanCount]
          ? props.bool[booleanCount].correct_answer
          : null,
        answers: [],
      };
    ...
    if (currentQuestionType === "text") {
      const questionObject = {
        ...
        correctAnswer: props.text[textCount]
          ? props.text[textCount].correct_answer
          : null,
        answers: [],
      };
    ...
    if (currentQuestionType === "multiple") {
      const questionObject = {
        question: props.multiple[multipleCount]
          ? props.multiple[multipleCount].question
          : null,
        correctAnswer: props.multiple[multipleCount]
          ? props.multiple[multipleCount].correct_answer
          : null,
        answers: props.multiple[multipleCount]
          ? props.multiple[multipleCount].answers
          : null,
      };
  ...
  const handleSelectedAnswer = (event: any, submittedOnce: boolean) => {
    ...
    if (!submittedOnce) {
      ...
      event.toLowerCase() === currentQuestionObject.correctAnswer.toLowerCase()
        ? newScoreData.correct++
        : newScoreData.wrong++;
      ...
      {!quizEnd && (
        <div>
          ...
          {currentQuestionType === "multiple" && (
            <Multiple
              question={currentQuestionObject && currentQuestionObject.question}
              answers={currentQuestionObject.answers}
              correctAnswer={currentQuestionObject.correctAnswer}
              handleSelectedAnswer={handleSelectedAnswer}
            />
          )}
